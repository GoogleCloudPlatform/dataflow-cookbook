...
from apache_beam.testing.test_stream import TimestampedValue
...
class FormatElements(DoFn):
  ...
  def process(self, element, window=DoFn.WindowParam, pane=DoFn.PaneInfoParam):
    yield 'Elements in pane: {} \nWindow {} - Pane Index {}\n' \
          .format(element[1], window, pane.index)
...
def run():
  ...
  (p | stream
     ...
     | "Window into Discarding FixedWindows" >> WindowInto(
