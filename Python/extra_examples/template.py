...
from apache_beam import DoFn, Map, ParDo, Pipeline
...
class CustomTemplateOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        parser.add_value_provider_argument(
            "--query", default=_QUERY, help="Query to read"
        )
        parser.add_value_provider_argument(
            "--min_population", default=0, help="Min Population"
        )
...
def run():
    ...
    (
        p
        | "Read from BQ"
        >> bigquery.ReadFromBigQuery(
            query=options.query, use_standard_sql=True, validate=False
        )
        ...
        | "Filter the elements"
        >> ParDo(FilterByPopulation(options.min_population))
