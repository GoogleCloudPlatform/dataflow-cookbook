...
object ListSideInput {
  ...
  def main(cmdlineArgs: Array[String]): Unit = {
    ...
    val rateSide = ratesSCol.asListSideInput // Creates a SideInput as List

    values
      .withSideInputs(rateSide) // point to side input (List)
      .map { (kv, s) =>
        // kv is the main value, s the side input
        // to retrieve the side input we need, we do s(SideInput Variable)
        val rate: Double = s(rateSide).filter { r => r._1.equals(kv._1) }(0)._2
