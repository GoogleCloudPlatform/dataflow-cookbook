...
object CombinePerKeyInterface {
  ...
  def main(cmdlineArgs: Array[String]): Unit = {
    ...
    val create: SCollection[Int] = sc.parallelize(1 to 30)

    create
      .withName("calculateDivisors")
      .flatMap { n => (1 to n).filter { x => n % x == 0 }.map { x => (n, x) } }
      // first _ + _ is adding the values, second _ + _ is adding the accumulators
      // 0.0 is the first accumulator
