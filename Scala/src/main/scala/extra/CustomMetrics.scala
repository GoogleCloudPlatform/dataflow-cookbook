...
import com.spotify.scio.{Args, ContextAndArgs, ScioContext, ScioMetrics}
import io.circe._
...
object CustomMetrics {
  ...
  def main(cmdlineArgs: Array[String]): Unit = {
    ...
    // Custom metrics
    val counterEnroute = ScioMetrics.counter("enroute")
    val counterDropoff = ScioMetrics.counter("dropoff")
    val counterPickup = ScioMetrics.counter("pickup")
    val counterCurrent = ScioMetrics.counter("current")
    ...
    pubsubRead
      ...
      .map { t: Taxi =>
        t.ride_status match {
          case "dropoff" => counterDropoff.inc(); counterCurrent.dec()
          case "pickup" => counterPickup.inc(); counterCurrent.inc()
          case "enroute" => counterEnroute.inc()
        }
        t
      }
